...
export async function readProjects(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM projects ORDER BY created_at DESC")
  return rows.map((r) => ({
    id: r.id,
    title: r.title,
...
export async function readServices(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM services ORDER BY display_order ASC, order_index ASC")
  return rows.map((r) => ({
    id: r.id,
    title: r.title,
    slug: r.slug,
...
export async function writeServices(list: unknown[]): Promise<void> {
  ...
  for (const r of list as Record<string, unknown>[]) {
    const features = r.features != null ? JSON.stringify(r.features) : null
    const benefits = r.benefits != null ? JSON.stringify(r.benefits) : null
    await p.query(
      ...
      [
        ...
        r.order_index ?? 0,
        r.status ?? "active",
        (r.is_active as boolean) !== false ? 1 : 0,
        r.created_by ?? null,
        r.updated_at ?? new Date().toISOString(),
        r.updated_at ?? new Date().toISOString(),
      ]
...
export async function writeBlog(list: unknown[]): Promise<void> {
  ...
  for (const r of list as Record<string, unknown>[]) {
    const tags = r.tags != null ? JSON.stringify(r.tags) : null
    await p.query(
...
export async function readMessages(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM contact_submissions ORDER BY created_at DESC")
  return rows.map((r) => ({
    id: r.id,
    name: r.name,
    email: r.email,
...
export async function readTestimonials(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM testimonials ORDER BY created_at DESC")
  return rows.map((r) => ({
    id: r.id,
...
// --- settings (company_info, una sola fila)
...
export async function readSettings(): Promise<Record<string, unknown> | null> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM company_info WHERE id = ? LIMIT 1", [COMPANY_INFO_ID])
  const r = rows[0]
  if (!r) return null
  ...
  await query(
    `INSERT INTO company_info (id, company_name, tagline, description, mission, vision, values, address, phone, email, whatsapp, facebook_url, instagram_url, linkedin_url, twitter_url, youtube_url, founded_year, cuit, logo_url, extra, updated_at)
     ...
     ON DUPLICATE KEY UPDATE
       company_name = VALUES(company_name), tagline = VALUES(tagline), description = VALUES(description),
       mission = VALUES(mission), vision = VALUES(vision), values = VALUES(values),
       address = VALUES(address), phone = VALUES(phone), email = VALUES(email), whatsapp = VALUES(whatsapp),
       facebook_url = VALUES(facebook_url), instagram_url = VALUES(instagram_url), linkedin_url = VALUES(linkedin_url),
       twitter_url = VALUES(twitter_url), youtube_url = VALUES(youtube_url), founded_year = VALUES(founded_year),
       cuit = VALUES(cuit), logo_url = VALUES(logo_url), extra = VALUES(extra), updated_at = NOW()`,
    [
      COMPANY_INFO_ID,
...
export async function readHeroImages(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM hero_images ORDER BY page, order_index ASC")
  return rows.map((r) => ({
    id: r.id,
    page: r.page,
    image_url: r.image_url,
...
export async function readCertifications(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT * FROM certifications ORDER BY order_index ASC")
  return rows.map((r) => ({
    id: r.id,
    name: r.name,
    logo_url: r.logo_url ?? null,
...
export async function readAdmins(): Promise<unknown[]> {
  const rows = await query<RowDataPacket[]>("SELECT id, email, full_name, role, password_hash FROM users ORDER BY email")
  return rows.map((r) => ({
    id: r.id,
    email: r.email,
...
// Actualizar is_read en un mensaje (contact_submissions)
export async function markContactRead(id: string): Promise<void> {
  await query("UPDATE contact_submissions SET is_read = 1 WHERE id = ?", [id])
}
